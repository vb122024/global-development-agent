...
| Ask agent | [`App.jsx`](../frontend/src/App.jsx) → [`agentAdapter.js`](../frontend/src/lib/agentAdapter.js) | Static: [`demoData.js`](../frontend/src/data/demoData.js). Local: proxy → `POST` [`main.py`](../backend/app/main.py). | Local casual chat uses one low-cost model turn with no tools or specialist handoff. Development-data questions, including short GDP questions, use the full agent workflow. The response appears in the read-only **Agent answer** text area. |
